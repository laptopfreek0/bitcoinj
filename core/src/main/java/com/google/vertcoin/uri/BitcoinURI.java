...
import com.google.vertcoin.core.AddressFormatException;
 ...
 *
 * <ul>
 * <li>{@code vertcoin:<address>}</li>
 * <li>{@code vertcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
...
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "vertcoin";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        // URI is formed as  vertcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form vertcoin://address?....
        ...
        if (input.startsWith("vertcoin://")) {
            schemeSpecificPart = input.substring("vertcoin://".length());
        } else if (input.startsWith("vertcoin:")) {
            schemeSpecificPart = input.substring("vertcoin:".length());
        } else {
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the vertcoin: prefix");
